...
2. 기존 planning 상태를 `absent`, `template-only`, `stale`, `partial`, `usable` 중 하나로 분류합니다.
3. `.planning/codebase/**`를 실제 저장소 기준으로 채웁니다.
4. active `.planning/phases/**` 문서 세트를 만들거나 현재 프로젝트 기준으로 보정합니다.
5. 이전 프로젝트나 템플릿 잔여 planning 문서를 `keep`, `archive`, `delete candidate`, `needs-human`으로 분류합니다.
6. `.planning/STATE.md`, `.planning/ROADMAP.md`, `.planning/DECISIONS.md`, `.planning/VERIFICATION.md`를 현재 프로젝트 기준으로 동기화합니다.
7. `.scratch/phase-state.json`은 명시적으로 phase-state 수정을 요청받았거나 phase-state repair가 범위일 때만 갱신합니다.
...
4. 구현 가능한 단위는 `/issues` + `workflow-docs-to-issues`로 vertical slice issue로 쪼갭니다.
5. 각 slice에 추천 owner mode를 붙입니다.
6. DB, ETL, ops, 일반 feature가 섞여 있으면 한 issue에 몰아넣지 않고 workflow별로 나눕니다.
7. `.planning/ROADMAP.md`와 active phase checkpoint에 현재 위치를 반영합니다.
...
설계 결정 workflow입니다. 문제, 제약, 비목표, 선택지, tradeoff를 정리하고 durable decision이 필요하면 ADR로 남깁니다. `.planning/codebase/**` 또는 active `.planning/phases/**`가 없거나 stale이면 먼저 `workflow-planning-hydration`으로 되돌립니다. 구현은 하지 않습니다. 구현이 필요하면 `/feature`, `/etl`, `/db`, `/ops` 중 적절한 workflow로 slice를 넘깁니다.
...
5. `plan`이라면 `.planning/phases/.../NN-01-PLAN.md`에 stage, acceptance criteria, allowed paths, verification을 씁니다.
6. 승인자가 `.scratch/phase-state.json`을 `phase=execute`, `approved=true`로 맞춥니다.
7. `execute`에서 `workflow-etl-pipeline`이 red test를 먼저 만듭니다.
8. parser/normalize/state 중 어느 stage가 바뀌는지 명시합니다.
9. 최소 구현 후 focused test를 통과시킵니다.
10. DB writer까지 영향을 주면 `/db` slice로 분리합니다.
11. 운영 신호가 필요하면 `/ops` slice로 분리합니다.
12. verification 결과를 phase 문서에 기록합니다.
13. `/review`로 read-only 리뷰를 수행합니다.
14. 모든 기준을 만족하면 `done`으로 닫고 `.planning/STATE.md`의 next action을 갱신합니다.
